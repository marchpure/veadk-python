...
- AskTable deterministic checks now explicitly support expected policy-deny cases while preserving the normal requirement that allowed queries return rows.
...
| --- | --- | --- | --- | --- |
| `semantic_skill` | `eval_run_99ecd429b22540fba3401ca5736fa815` | `succeeded` | `1` | SQL `True`, dashboard diff `False`, policy `True`, freshness `True`, evidence `True` |
| `asktable_query` | `eval_run_256eff2a005c48728756e77190ec3ad7` | `succeeded` | `1` | SQL `True`, dashboard diff `False`, policy `True`, freshness `True`, evidence `True`; imported 2 cases covering allow and PII-deny |
| `dashboard_skill` | `eval_run_043f00bc851a4f66b682be8b7753cf0e` | `succeeded` | `1` | SQL `True`, dashboard diff `True`, policy `True`, freshness `True`, evidence `True` |

AskTable case evidence:
- Allow case: `policyDecision=allow`, rows `2`, freshness `fresh`, raw SQL fallback `False`, evidence `ticket_count definition`.
- PII/contact case: `policyDecision=deny`, status `blocked`, rows `0`, freshness `blocked`, SQL marker `no raw SQL executed`, raw SQL fallback `False`, evidence `PII policy guard`.
...
- `uv run pytest tests/frontend/test_knowledge_asset_evaluation.py tests/frontend/test_knowledge_asset_store.py -q` -> 30 passed.
...
- Custom scan command: `python inline custom_sensitive_scan.py <git diff and requested G2 paths>`.
- Findings: `0`.
