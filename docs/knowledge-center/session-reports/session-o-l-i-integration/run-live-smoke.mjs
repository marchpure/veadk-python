...
const studioBaseUrl = (process.env.VEADK_STUDIO_URL || "http://127.0.0.1:18222").replace(/\/$/, "");
const sourceLHash = process.env.SOURCE_L_HASH || "7878adfa08a73f9214e5415b2f16114ddc5b5df6";
const sourceIHash = process.env.SOURCE_I_HASH || "2cfcc37b53380c9e96cfe33445140286eb9b789d";
const integrationBranch = process.env.INTEGRATION_BRANCH || gitText(["rev-parse", "--abbrev-ref", "HEAD"]) || "kc/session-o-l-i-integration";
const integrationBranchHash = process.env.INTEGRATION_BRANCH_HASH || gitText(["rev-parse", "HEAD"]);
...
async function seedWorkbench() {
  ...
  const space = await api("/api/knowledge-assets/spaces", {
    method: "POST",
    body: JSON.stringify({
      name: `Session O L I ${unique}`,
      description: "Live smoke fixture for Session O L+I integration.",
    }),
  });
  ...
  const resource = await api("/api/knowledge-assets/source-resources", {
    ...
    body: JSON.stringify({
      ...
      content_hash: "session-o-local-fixture",
      tags: ["session-o", "schema", "oracle"],
  ...
  const snapshot = await api("/api/knowledge-assets/snapshots", {
    ...
    body: JSON.stringify({
      ...
      profile: { snapshot: { id: `${unique}-snapshot`, hash: "session-o-local-fixture" } },
    }),
  });
  ...
  await api("/api/knowledge-assets/skill-packages", {
    ...
    body: JSON.stringify({
      ...
      name: "Sales Dashboard Session O",
      status: "ready",
      publish_state: "published",
  ...
  const specs = [
    {
      ...
      name: `Session O Semantic Skill Eval`,
      cases: [
        {
          targetKind: "semantic_skill",
          question: "按门店查看销售票数",
    ...
    },
    {
      ...
      targetAssetId: semanticAssetId,
      name: `Session O AskTable Query Eval`,
      cases: [
        {
          targetKind: "asktable_query",
          question: "按门店查看销售票数",
        ...
        },
        {
          ...
          tags: ["session-o", "asktable", "policy-deny"],
        },
      ],
    },
    {
      ...
      name: `Session O Dashboard Skill Eval`,
      cases: [
        {
          targetKind: "dashboard_skill",
  ...
  for (const spec of specs) {
    const suite = await api("/api/knowledge-assets/evaluation/suites", {
      ...
      body: JSON.stringify({
        spaceId: space.id,
        name: spec.name,
        description: "Session O deterministic evaluation suite imported from JSON cases.",
...
async function verifyUi(seed) {
  ...
  try {
    for (const viewport of viewports) {
      ...
      await page.getByTestId("semantic-builder-workspace").waitFor({ state: "visible" });
      await page
        .getByTestId("semantic-builder-workspace")
        .locator(".kc-builder-control > span", { hasText: /^Semantic Skill$/ })
        .waitFor({ state: "visible" });
      await page
        .getByTestId("semantic-builder-workspace")
        .locator("select")
        .first()
        .selectOption(seed.semanticAssetId);
      const selectedSemanticAssetId = await page
        .getByTestId("semantic-builder-workspace")
        .locator("select")
        .first()
        .inputValue();
      if (selectedSemanticAssetId !== seed.semanticAssetId) {
        throw new Error(`Semantic Builder did not select seeded asset ${seed.semanticAssetId}; selected ${selectedSemanticAssetId}`);
      }
      await page.getByTestId("wren-source-port-diagram").waitFor({ state: "visible" });
      const semanticRendered = await page.getByTestId("semantic-builder-workspace").isVisible();
      ...
      const asktableRendered = await page.getByTestId("ask-dashboard-workbench").isVisible();
      ...
      await page.getByText("Session O Semantic Skill Eval").waitFor({ state: "visible" });
      await page.getByText("Session O AskTable Query Eval").waitFor({ state: "visible" });
      await page.getByText("Session O Dashboard Skill Eval").waitFor({ state: "visible" });
      ...
      await page.getByRole("button", { name: /Session O AskTable Query Eval/ }).click();
      ...
      await page.getByRole("button", { name: /Session O Dashboard Skill Eval/ }).click();
      await page.waitForFunction(() => {
        const detail = document.querySelector(".kc-eval-run-detail")?.textContent || "";
      ...
      const evaluationRendered = await page.locator(".kc-eval-workbench").isVisible();
...
async function inspectLayout(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      hasByaanSidebar: /BYAAN|Data Studio sidecar/i.test(navText),
      hasWrenSidebar: /Wren AI|Wren Engine/i.test(navText),
      semanticRendered: Boolean(document.querySelector('[data-testid="semantic-builder-workspace"]')),
...
function validateConnectors(connectors) {
  if (connectors.schema_version !== "knowledge_asset.connector_registry.v1" || connectors.mock !== false) {
    throw new Error(`unexpected connector registry envelope: ${JSON.stringify(connectors)}`);
  }
  const ids = new Set((connectors.items || []).map((item) => item.id));
  for (const id of ["oracle", "feishu_doc", "web", "file", "text", "pdf", "image"]) {
    if (!ids.has(id)) {
      throw new Error(`connector registry missing ${id}: ${JSON.stringify(connectors)}`);
    }
  }
}

const health = await api("/api/knowledge-assets/health");
const connectors = await api("/api/knowledge-assets/connectors");
...
const report = {
  schema: "agentkit.knowledge_center.session_o_l_i_integration.v1",
  generatedAt: new Date().toISOString(),
  studioUrl: studioBaseUrl,
  sourceBranchHashes: {
    "kc/session-l-unified-knowledge-assets": sourceLHash,
    "kc/session-i-connectors-foundation": sourceIHash,
  },
  ...
  connectorRegistry: {
    schema_version: connectors.schema_version,
    mock: connectors.mock,
    total: connectors.total,
    requiredIds: ["oracle", "feishu_doc", "web", "file", "text", "pdf", "image"],
  },
  ...
  mergeBase: {
    createdFrom: "origin/kc/session-l-unified-knowledge-assets",
    baseCommit: sourceLHash,
    mergedBranch: "origin/kc/session-i-connectors-foundation",
    mergedCommit: sourceIHash,
    strategy: `merge --no-ff into ${integrationBranch}`,
  },
  ...
  testCommands: [
    ...
    {
      command: "uvx ruff check frontend/server/knowledge_assets tests/frontend",
      status: "passed",
      summary: "All checks passed.",
    },
    {
      command: "python -m pytest tests/frontend/test_knowledge_asset_routes.py tests/frontend/test_knowledge_asset_store.py tests/frontend/test_dashboard_askdata_routes.py tests/frontend/test_knowledge_asset_agents.py tests/frontend/test_knowledge_asset_semantic_builder.py tests/frontend/test_knowledge_asset_evaluation.py -q",
      status: "passed",
      summary: "78 passed, 7 warnings.",
    },
    {
      command: "cd frontend && npm test -- knowledgeAssetWorkbench.test.mjs semanticBuildPanel.test.mjs",
      status: "passed",
      summary: "682 passed, 0 failed after npm ci.",
    },
    {
    ...
    {
      command: "VEADK_STUDIO_URL=http://127.0.0.1:18222 node docs/knowledge-center/session-reports/session-o-l-i-integration/run-live-smoke.mjs",
      status: "passed",
