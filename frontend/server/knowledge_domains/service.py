...
from frontend.server.knowledge_assets.contract_data import GoldenAssetRevision, SkillDraftRevision
from frontend.server.knowledge_assets.kind_runtime import ExecutionBudget, KindExecutionRequest
from frontend.server.knowledge_assets.kind_runtime.handlers import KnowledgeHandler
from frontend.server.knowledge_assets.kind_runtime.tabular import infer_fields, parse_rows
...
def _extract_text(filename: str, content: bytes) -> str:
    ...
    if suffix == ".pdf":
        ...
        try:
            ...
            extraction_errors.append(exc)
        raise ValueError("PDF 文本提取失败，请检查文件内容或服务端 PDF 依赖。") from extraction_errors[-1]
...
def fetch_feishu_document(url: str, credential: str) -> dict[str, Any]:
    ...
    api_host = "open.larksuite.com" if hostname.endswith("larksuite.com") else "open.feishu.cn"
    endpoint = (
...
class DomainService:
    ...
    def create_knowledge_base(self, workspace_id: str, name: str, description: str, scope: str) -> dict[str, Any]:
        now = _now()
        kb_id = _id("kb")
        draft_id = _id("skill")
        manifest = {
            ...
            "spec": {"kind": "knowledge", "retrievalMode": "hybrid", "sourceRevisionRefs": []},
        }
        ...
        self.connection.commit()
        return {"id": kb_id, "name": name, "description": description, "scope": scope, "lifecycle": "draft",
                "skillDraft": {"id": draft_id, "revision": 1, "kind": "knowledge", "status": "draft", "manifest": manifest}}

    def _knowledge_base(self, knowledge_base_id: str) -> sqlite3.Row:
        row = self.connection.execute("SELECT * FROM knowledge_bases WHERE id = ?", (knowledge_base_id,)).fetchone()
        if row is None:
            raise KeyError("knowledge base not found")
        return row

    def knowledge_base_summary(self, knowledge_base_id: str) -> dict[str, Any]:
            ...
            "sources": [
                {
                    ...
                    "goldenAssetRevision": {
                        ...
                        "status": "ready",
                        "sourceRevisionId": item["source_revision_id"],
                    } if item["golden_asset_id"] else None,
                    "index": {
                        "status": "ready",
    ...
    def add_source(
        self, knowledge_base_id: str, *, filename: str, title: str, description: str,
        tags: str, media_type: str, content: bytes, chunk_strategy: str, source_type: str = "local",
        ...
        self.connection.execute(
            ...
            (source_id, knowledge_base_id, filename, title or filename, description, tags, media_type,
             content, text, source_type, digest, now),
        )
        self.connection.execute(
        ...
        self.connection.execute("UPDATE knowledge_bases SET updated_at = ? WHERE id = ?", (now, knowledge_base_id))
        self.connection.commit()
        return {
            "document": {"id": source_id, "knowledgeBaseId": knowledge_base_id, "title": title or filename,
                         "filename": filename, "content": text, "contentBytes": len(content)},
            "sourceRevision": {"id": source_revision_id, "revision": 1, "sourceDigest": digest, "traceId": trace,
                               "status": "ready"},
            "goldenAssetRevision": {"id": golden_id, "revision": 1, "status": "ready",
                                    "sourceRevisionId": source_revision_id},
            "index": {"status": "ready", "chunkCount": len(chunk_values), "indexedAt": now},
            "chunks": [{"id": row["id"], "locator": row["locator"], "text": row["text"]}
                       for row in self.connection.execute("SELECT id, locator, text FROM chunks WHERE source_id = ? ORDER BY chunk_index", (source_id,))],
            "skillDraft": {"id": draft["id"], "revision": draft["revision"]} if draft else None,
        }

    def standalone_document(self, workspace_id: str, **kwargs: Any) -> dict[str, Any]:
        created = self.create_knowledge_base(workspace_id, kwargs["title"] or kwargs["filename"], kwargs["description"], kwargs["scope"])
        result = self.add_source(created["id"], **kwargs)
        result["knowledgeBase"] = created
        return result

    def document(self, source_id: str) -> dict[str, Any]:
    ...
    def inspect_feishu(
        ...
        if not credential:
            return {"status": "credential_blocked", "url": url, "message": "飞书连接需要服务端凭证。"}
    ...
    def ask(self, knowledge_base_id: str, question: str, top_k: int) -> dict[str, Any]:
        ...
        rows = self.connection.execute(
            """SELECT c.id, c.text, c.locator, s.title, s.id AS source_id,
               ...
               JOIN source_revisions sr ON sr.source_id = s.id AND sr.revision = 1
               WHERE s.knowledge_base_id = ? ORDER BY c.chunk_index""", (knowledge_base_id,)
        ).fetchall()
        ranked = sorted(rows, key=lambda row: sum(term in row["text"].lower() for term in terms), reverse=True)
        selected = [row for row in ranked if not terms or any(term in row["text"].lower() for term in terms)][:max(1, top_k)]
        trace = _id("trace")
        skill_answer = self._run_knowledge_skill(knowledge_base, question, rows, trace)
        citations = [{"id": row["id"], "sourceId": row["source_id"], "title": row["title"],
                      "locator": row["locator"], "snippet": row["text"][:400]} for row in selected]
        ...
        return {
            ...
            "skill": {"kind": "knowledge", "runtime": "worker3-knowledge", "sourceCount": len(selected)},
        }

    def knowledge_query_result(self, knowledge_base_id: str, query_result_id: str) -> dict[str, Any]:
        row = self.connection.execute(
    ...
    def _run_knowledge_skill(
        ...
        return str(view_model.answer) if view_model is not None and hasattr(view_model, "answer") else None

    def publish(self, knowledge_base_id: str) -> dict[str, Any]:
        self._knowledge_base(knowledge_base_id)
        now = _now()
        self.connection.execute("UPDATE knowledge_bases SET lifecycle = 'published', updated_at = ? WHERE id = ?", (now, knowledge_base_id))
        self.connection.execute("UPDATE skill_drafts SET status = 'ready', updated_at = ? WHERE knowledge_base_id = ?", (now, knowledge_base_id))
        self.connection.commit()
        return {"knowledgeBaseId": knowledge_base_id, "lifecycle": "published", "publishedAt": now}

    @staticmethod
    def validate_mdl(mdl: str) -> dict[str, Any]:
        if not mdl.strip():
            return {"valid": False, "errors": [{"line": 1, "message": "MDL 不能为空。"}]}
        lines = mdl.splitlines()
        models = re.findall(r"^\s*model\s+([A-Za-z_]\w*)\s*\{", mdl, re.MULTILINE)
        if not models:
            return {"valid": False, "errors": [{"line": 1, "message": "缺少 model 声明。"}]}
        ...
        for line_no, raw_line in enumerate(lines, 1):
            ...
            if line.startswith("model "):
                ...
                if current_model is not None:
                    errors.append({"line": line_no, "message": "上一个 model 缺少结束括号。"})
            ...
            if line == "}":
                if current_model is None:
                    errors.append({"line": line_no, "message": "出现了未匹配的结束括号。"})
                else:
                    current_model = None
                depth -= 1
                continue
            ...
            if field_match:
                ...
                if name in fields:
                    errors.append({"line": line_no, "message": f"字段 {name} 重复定义。"})
                if field_type and field_type not in {"string", "number", "integer", "boolean", "date", "datetime", "calculated"}:
                    errors.append({"line": line_no, "message": f"字段类型 {field_type} 未注册。"})
                fields.add(name)
            ...
            if join_match:
                target, left_model, left_field, right_model, right_field, cardinality = join_match.groups()
                join = {
                    "target": target,
        ...
        for join in joins:
            ...
            if target_model not in model_names or left_model not in model_names or right_model not in model_names:
        ...
        for model in parsed_models:
            ...
            for field in model["fields"]:
                ...
                allowed = local_fields | {"true", "false", "null", "sum", "count", "min", "max", "avg"}
                unknown = sorted(identifier for identifier in identifiers if identifier not in allowed)
        ...
        return {"valid": not errors, "errors": errors, "models": models, "fields": sorted(fields), "schema": schema}

    def get_semantic(self, resource_id: str) -> dict[str, Any]:
        row = self.connection.execute("SELECT * FROM semantic_models WHERE resource_id = ?", (resource_id,)).fetchone()
        if row is None:
            return {
    ...
    def source_revisions(self) -> list[dict[str, Any]]:
        rows = self.connection.execute(
    ...
    def _semantic_golden_asset(
        ...
        return GoldenAssetRevision(
            ...
            owner=OwnerRef(workspace_id="workspace-semantic", principal_id="knowledge-domain"),
            permissions_ref=PermissionRef(uri="permission://workspace/semantic/read", version="1"),
            lineage_digest=digest,
            freshness_at=now,
        )
    ...
    def save_semantic(
        ...
        current = self.connection.execute("SELECT * FROM semantic_models WHERE resource_id = ?", (resource_id,)).fetchone()
        current_revision = current["revision"] if current else 0
        if current_revision != expected_revision:
            raise RuntimeError("semantic revision conflict")
        revision = current_revision + 1
        old_fields = set(self.validate_mdl(current["mdl"])["fields"]) if current else set()
        new_fields = set(validation["fields"])
        diff = {"addedFields": sorted(new_fields - old_fields), "removedFields": sorted(old_fields - new_fields)}
        impact = {"affectedAssets": [], "warnings": ["下游影响分析基于当前服务端注册资产。"]}
        now = _now()
        golden_asset = source_golden or self._semantic_golden_asset(resource_id, revision, mdl, validation, now)
        self.connection.execute(
        ...
        self.connection.execute("INSERT INTO semantic_revisions VALUES (?, ?, ?, ?, ?, ?, ?)",
                                (_id("sem_rev"), resource_id, revision, mdl, json.dumps(diff), json.dumps(impact), now))
        self.connection.execute(
            ...
            (
                ...
                resource_id,
                revision,
                json.dumps(golden_asset.model_dump(mode="json", by_alias=True), ensure_ascii=False),
    ...
    def graph(self, resource_id: str) -> dict[str, Any]:
        row = self.connection.execute("SELECT * FROM graph_projections WHERE resource_id = ?", (resource_id,)).fetchone()
        if row is None:
            return {"resourceId": resource_id, "revision": 0, "entities": [], "relationships": [], "constraints": [], "lineage": []}
        projection = json.loads(row["projection"])
        return {"resourceId": resource_id, "revision": row["revision"], **projection}

    def mutate_graph(self, resource_id: str, mutation: dict[str, Any]) -> dict[str, Any]:
        current = self.graph(resource_id)
        ...
        if operation == "upsert_entity":
            ...
            entities = [item for item in entities if item.get("id") != entity["id"]] + [entity]
            ...
            relationships = [item for item in relationships if item.get("id") != relationship["id"]] + [relationship]
        else:
            raise ValueError("不支持的图谱变更。")
        revision = int(current["revision"]) + 1
        projection = {"entities": entities, "relationships": relationships,
                      "constraints": current.get("constraints", []), "lineage": current.get("lineage", [])}
        if operation == "upsert_entity":
            for constraint in entity.get("constraints", []):
                projection["constraints"].append({"entityId": entity["id"], "constraint": constraint})
            target_id = entity["id"]
        else:
        ...
        self.connection.commit()
        return {"resourceId": resource_id, "revision": revision, **projection}

    def query_graph(self, resource_id: str, query: dict[str, Any]) -> dict[str, Any]:
        projection = self.graph(resource_id)
        ...
        if mode == "path":
            ...
            path_nodes = [start]
            path_nodes.extend(
                relationship["to"] for relationship in path
        ...
        neighbors = [item for item in projection["relationships"] if item.get("from") == entity_id or item.get("to") == entity_id]
        result = {"queryResultId": _id("graph_query"), "mode": "neighbors", "entityId": entity_id,
                "relationships": neighbors,
                "entities": sorted({entity_id, *[
                    endpoint for relationship in neighbors
    ...
    def graph_query_result(self, resource_id: str, query_result_id: str) -> dict[str, Any]:
        row = self.connection.execute(
