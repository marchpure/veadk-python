...
class KindRuntime:
    ...
    def retry(
        ...
        original = (
            self.repository.get(retry_of_operation_id) if self.repository else None
        )
        if original is None:
    ...
    def _execute(
        ...
        *,
        ...
        if (
            request.draft_revision.manifest.metadata.owner.workspace_id
            != request.workspace_id
        ):
            trace.steps.append("workspace-denied")
            return self._complete(
                self._terminal(
                    request,
                    operation_id=operation_id,
                    ...
                    handler="none",
                    message="Skill revision does not belong to the execution workspace.",
                    retry_of_operation_id=retry_of_operation_id,
                )
            )
        if any(
            item.owner.workspace_id != request.workspace_id
            for item in request.golden_asset_revisions
        ):
            trace.steps.append("golden-workspace-denied")
            return self._complete(
                self._terminal(
                    request,
                    operation_id=operation_id,
                    ...
                    state="permission_denied",
                    handler="none",
                    message="Golden Asset revision does not belong to the execution workspace.",
                    retry_of_operation_id=retry_of_operation_id,
                )
            )
        ...
        byte_count = sum(
            ...
            for value in request.golden_asset_contents.values()
        )
        ...
        try:
            ...
            while True:
                if self.repository is not None and self.repository.cancel_requested(
                    operation_id
                ):
        ...
        trace.steps.extend(["project-view-model", "write-content-addressed-revisions"])
        ...
        skill_result, view_intent, view_revision, evidence_ref, _ = (
            self.projector.project(request, output)
        )
        ...
        if kind == "monitoring":
            ...
            observations = [
                MonitoringObservation(
                    ...
                    change_rate=float(item["changeRate"])
            ...
            action_candidates = [
                MonitoringActionCandidate(
                    ...
                    evidence_locator=str(
                        item.get("evidenceLocator", "monitoring:latest")
                    ),
                )
    ...
    def _operation_id(self, request: KindExecutionRequest) -> str:
        ...
        return (
            "w3op-" + hashlib.sha256(request.idempotency_key.encode()).hexdigest()[:24]
        )
