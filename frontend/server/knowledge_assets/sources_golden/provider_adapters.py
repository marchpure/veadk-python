    ...
    def _connect(self, request: ConnectorRequest):
        ...
        if key in {"doris", "starrocks"}:
            ...
            with connection.cursor() as cursor:
                ...
                if key in {"doris", "starrocks"}:
                    # Doris-family MySQL protocols accept the session and
                    # transaction read-only settings, but do not implement
                    # START TRANSACTION READ ONLY. Start the transaction
                    # after the settings so the adapter remains fail-closed
                    # without issuing a dialect-invalid statement.
