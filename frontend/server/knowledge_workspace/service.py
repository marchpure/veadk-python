...
class KnowledgeWorkspaceService:
    ...
    def _template_requirements(cls, template_key: TemplateKey) -> str:
        shared = (
            ...
            "- Keep the Skill self-contained and portable: use Python standard "
            "library only in scripts/tests, do not add psycopg/psycopg2, "
            "requests, jinja, or packages that require network installation. "
            "Use the authorized MCP/runtime action for live data and pass "
            "captured, redacted results into the Skill's deterministic logic.\n"
            "- Before validate_skill, write every declared target file at the "
            "exact declared path under the Skill directory; do not rename or "
            "substitute files after declaring them.\n"
            "- Tests must be self-consistent with the final files: import only "
            "functions/classes that are actually defined in the current Skill "
            "scripts, never symbols from an earlier draft or an imagined helper. "
            "Run `python -m pytest tests -q` from the Skill root after all files "
            "are written. If collection, import, syntax, or assertion errors "
            "occur, inspect the final files and fix them before calling "
            "validate_skill; do not repeatedly call validate_skill on unchanged "
            "files.\n"
        ...
        template = {
            ...
            TemplateKey.DASHBOARD: (
                ...
                "- Use a self-contained real-data snapshot and local inline "
                "JavaScript event listeners for interaction; do not use inline "
                "event attributes such as onclick, external scripts, fetch, "
                "cookies, browser storage, or timers that call a remote service.\n"
    ...
    @staticmethod
    def _state_with_skill(
        state: bytes | None,
        skill_zip: bytes,
        *,
        skill_name: str,
    ) -> bytes:
        """Add one validated immutable Skill to the stateless request state."""

        skill_prefix = f"skillhub/{skill_name}/"
            ...
            with source:
                for info in source.infolist():
                    ...
                    if name.startswith(skill_prefix):
                        continue
                    entries[name] = source.read(info)
        try:
            skill_archive = zipfile.ZipFile(io.BytesIO(skill_zip))
        except zipfile.BadZipFile as exc:
            raise KnowledgeWorkspaceError(
                "SKILL_ZIP_INVALID", "revision Skill ZIP is not a valid ZIP", 502
            ) from exc
        with skill_archive:
            for info in skill_archive.infolist():
                if info.is_dir():
                    continue
                name = info.filename
                if not name.startswith(skill_prefix):
                    raise KnowledgeWorkspaceError(
                        "SKILL_ZIP_INVALID",
                        "revision Skill ZIP contains an unexpected root",
                        502,
                    )
                if (
                    not name
                    ...
                    raise KnowledgeWorkspaceError(
                        "SKILL_ZIP_INVALID",
                        "revision Skill ZIP contains an unsafe path",
                        502,
                    )
                if name.casefold() in {item.casefold() for item in entries}:
                    raise KnowledgeWorkspaceError(
                        "AUTOSKILL_STATE_INVALID",
                        "Skill path collides with existing AutoSkill state",
                        502,
                    )
                entries[name] = skill_archive.read(info)
        output = io.BytesIO()
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
            ...
            if (
                ...
                if prepare_autoskill is not None:
                    ...
                    if (
                        ...
                        prepared_autoskill_state = prepared_state
            ...
            if (
                ...
                if is_stateless:
                    ...
                    combined_state = self._state_with_skill(
                        ...
                        if prepared_autoskill_state is not None
                        else (
                            session_state
                            if (
                                invocation.autoskill_agent_id
                    ...
                    prepared_autoskill_state = combined_state
                    if (
                        invocation.autoskill_agent_id == session.autoskill_agent_id
                        and invocation.autoskill_session_id
                        == session.autoskill_session_id
