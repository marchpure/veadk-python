    ...
    with archive:
        ...
        for info in infos:
            ...
            if stat.S_IFMT(mode) in {stat.S_IFLNK, stat.S_IFCHR, stat.S_IFBLK, stat.S_IFIFO, stat.S_IFSOCK}:
                raise HtmlArtifactError("ARTIFACT_OUTPUT_INVALID", "output ZIP contains a special file")
            if info.is_dir():
                continue
            files += 1
            if files > max_files:
                raise HtmlArtifactError("ARTIFACT_OUTPUT_TOO_LARGE", "output ZIP contains too many files")
            expanded += info.file_size
            if info.file_size > max_file_bytes:
                raise HtmlArtifactError("ARTIFACT_OUTPUT_TOO_LARGE", "output ZIP contains an oversized file")
            ...
            if info.compress_size and info.file_size > info.compress_size * max_compression_ratio:
                raise HtmlArtifactError("ARTIFACT_OUTPUT_TOO_LARGE", "output ZIP has a suspicious compression ratio")
        candidates = [
            i for i in infos
