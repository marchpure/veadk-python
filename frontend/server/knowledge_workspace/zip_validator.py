...
def validate_skill_zip(
    ...
    *,
    ...
    if len(content) > max_archive_bytes:
        raise SkillZipError("SKILL_ZIP_TOO_LARGE", "Skill ZIP exceeds compressed size limit")
    try:
        archive = zipfile.ZipFile(io.BytesIO(content))
    except zipfile.BadZipFile as exc:
        raise SkillZipError("SKILL_ZIP_INVALID", "Skill ZIP is not a valid ZIP archive") from exc
    ...
    with archive:
        ...
        for info in infos:
            ...
            if normalized.casefold() in folded:
                raise SkillZipError("SKILL_ZIP_DUPLICATE_PATH", f"duplicate ZIP path: {raw}")
            ...
            if file_type in {stat.S_IFLNK, stat.S_IFCHR, stat.S_IFBLK, stat.S_IFIFO, stat.S_IFSOCK}:
                raise SkillZipError("SKILL_ZIP_SPECIAL_FILE", f"special ZIP entry: {raw}")
            if info.is_dir() or raw.endswith("/"):
                continue
            files += 1
            if files > max_files:
                raise SkillZipError("SKILL_ZIP_FILE_COUNT", "Skill ZIP contains too many files")
        ...
        if root[0] != "skillhub" or len(root) != 2 or not root[1]:
            raise SkillZipError("SKILL_ZIP_ROOT", "Skill ZIP root must be skillhub/<name>/")
        skill_path = f"{root[0]}/{root[1]}/SKILL.md"
        if skill_path not in paths:
            raise SkillZipError("SKILL_MD_MISSING", "Skill ZIP must contain a non-empty SKILL.md")
