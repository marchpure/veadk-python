...
export default function AddKnowledgeBaseView({ searchParams, setSearchParams, showToast }: any) {
  ...
  const [feishuState, setFeishuState] = useState<'idle'|'checking'|'ready'|'syncing'|'done'>('idle');
  ...
  const handleLocalUpload = (fileName: string) => {
    setUploadState('uploading');
    setTimeout(() => {
      setSources([...sources, { id: Date.now().toString(), type: 'local', name: fileName, status: 'ready', chunks: 12, size: '2.4MB', time: '刚刚' }]);
      setUploadState('idle');
      showToast?.(`已成功解析并添加 ${fileName}`);
    }, 1500);
  };
  
  const handleFeishuCheck = () => {
    if(!feishuUrl) return;
  ...
  const handleFeishuSync = () => {
    ...
    setTimeout(() => {
      setSources([...sources, { id: Date.now().toString(), type: 'feishu', name: '销售退货流程指引', status: 'ready', chunks: 8, url: feishuUrl, size: '在线文档', time: '刚刚' }]);
      setFeishuState('idle');
      setFeishuUrl('');
      showToast?.('飞书文档同步成功');
    }, 1500);
  ...
  const handleCreate = () => {
    ...
    const newKb = {
      ...
      lifecycle: 'draft',
      permission: true,
      capabilities: ['queryable', 'searchable', 'shareable'],
      lineage: { sourceIds: [] },
      createdAt: '刚刚',
      updatedAt: '刚刚',
      tokenEstimate: sources.reduce((acc, s) => acc + s.chunks * 0.1, 0),
      // legacy compat
      name: name,
      type: 'personal_artifact',
      artifactType: 'knowledge_base',
      sources: sources
    };
    
    resourceStore.setState(prev => [newKb as any, ...prev]);
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 flex flex-col h-full">
                ...
                {activeTab === 'local' && (
                  <div className="space-y-4">
                    <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center hover:bg-slate-50 transition-colors relative cursor-pointer min-h-[160px]">
                      <input type="file" onChange={(e) => { if(e.target.files?.[0]) handleLocalUpload(e.target.files[0].name); }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                      {uploadState === 'uploading' ? (
                        ...
                        <div className="flex flex-col items-center text-slate-500">
                          ...
                          <span className="text-xs mt-2 text-blue-600 font-medium hover:underline relative z-10" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleLocalUpload('sample-sales-policy.pdf'); }}>使用样例 sample-sales-policy.pdf 一键填充</span>
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    {feishuState === 'ready' && (
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 animate-in slide-in-from-top-2">
                        <div className="flex justify-between items-center mb-3 pb-3 border-b border-slate-200">
                          <div>
                            <div className="font-bold text-slate-800">销售退货流程指引</div>
                            <div className="text-xs text-slate-500 mt-1">所有者: 张经理 | 更新于 2小时前</div>
                          </div>
                ...
                <div className="flex justify-between pt-4 border-t border-slate-100">
                  ...
                  <button onClick={handleCreate} disabled={sources.length === 0} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-bold shadow-sm hover:bg-blue-700 disabled:opacity-50 outline-none">完成创建</button>
                </div>
              </div>
            )}
          </div>
