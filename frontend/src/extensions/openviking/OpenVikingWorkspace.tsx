...
function returnToSkillCreator(profile: OpenVikingProfile) {
  sessionStorage.setItem(
    KNOWLEDGE_SOURCE_RETURN_OPTION_KEY,
    `openviking:profile:${profile.profile_id}`,
  )
  const target = new URL(window.location.href)
  target.search = ''
  target.searchParams.set('view', 'knowledge-workspace')
  target.searchParams.set('file', 'skill_new')
                  ...
                  rightContent={activeProfile ? (
                    <div className="ov-navbar-profile">
                      <button
                        ...
                        onClick={() => returnToSkillCreator(activeProfile)}
                      >
                        加入 Skill 上下文
                      </button>
