...
import { BarChart3, Check, Database, Info, Loader2, Plus, Search } from "lucide-react";
import {
  DataStudioError,
...
function toSelected(hit: SkillHit): SelectedSkill {
  return {
    ...
    dataStudioMcpUrl: hit.dataStudioMcpUrl,
      ...
      {loading && hits.length === 0 ? (
        ...
        <>
          <div className="cw-datastudio-grid">
            {hits.map((hit) => {
              const on = selectedKeys.has(`${hit.dataStudioAssetType}:${hit.dataStudioAssetId}`);
              const Icon = hit.dataStudioAssetType === "dashboard" ? BarChart3 : Database;
              return (
                <button
                  key={hit.id}
                  type="button"
                ...
                >
                  <span className="cw-datastudio-card-head">
                    ...
                    <span className="cw-skill-result-icon" aria-hidden>
                      {on ? <Check className="cw-i cw-i-sm" /> : <Plus className="cw-i cw-i-sm" />}
                    </span>
                  </span>
                  ...
                  <span className="cw-datastudio-meta">
                    门禁 {hit.dataStudioGateScore ?? "N/A"} · {hit.dataStudioVersion || "published"}
                  </span>
                  <span className="cw-datastudio-meta">
                    指标 {hit.dataStudioMetrics?.length ?? 0}
                    {hit.dataStudioMetrics?.length ? `：${hit.dataStudioMetrics.slice(0, 3).join("、")}` : ""}
                  </span>
                  <span className="cw-datastudio-example">
                    {hit.dataStudioExampleQuestions?.[0] ?? "可围绕该资产覆盖的指标和维度提问"}
                  </span>
