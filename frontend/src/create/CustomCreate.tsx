...
import { codegenDraft } from "./codegenDraft";
import {
            ...
            >
              ...
              <div className="cw-skill-dialog-body">
                ...
                >
                  ...
                  {active === "datastudio" && (
                    <DataStudioAssetPicker
                      selected={dataAssets}
                      onChange={onDataAssetsChange}
                    />
                  )}
                </div>
              </div>
...
function collectDeploymentEnv(root: AgentDraft): RuntimeEnvConfiguration {
  ...
  const visit = (node: AgentDraft) => {
    ...
    if ((node.dataAssets ?? []).some((asset) => asset.source === "datastudio")) {
      selections.push({
        env: [
          {
            key: "BYAAN_MCP_API_KEY",
            required: true,
            placeholder: "your-mcp-auth-token",
            comment: "Byaan Data Studio MCP 鉴权 Token",
          },
        ],
      });
    }
...
export function CustomCreate({
  ...
  const dataAssets = node.dataAssets ?? [];
                            ...
                            <Section meta={metaOf("skills")}>
                              <div className="cw-form">
                                <SkillsSourceTabs
                                  ...
                                  onDataAssetsChange={(next) =>
                                    patch({ dataAssets: next })
                                  }
                                  cloudProvider={cloudProvider}
                                />
                              </div>
