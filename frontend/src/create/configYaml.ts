...
function toDataAssetConfig(skill: NonNullable<AgentDraft["dataAssets"]>[number]) {
  const base: Record<string, unknown> = {
    source: "datastudio",
    name: skill.name,
    folder: skill.folder,
    dataStudioAssetType: skill.dataStudioAssetType,
    dataStudioAssetId: skill.dataStudioAssetId,
  };
  if (skill.description) base.description = skill.description;
  if (skill.dataStudioVersion) base.dataStudioVersion = skill.dataStudioVersion;
  if (typeof skill.dataStudioGateScore === "number") {
    base.dataStudioGateScore = skill.dataStudioGateScore;
  }
  if (skill.dataStudioMetrics?.length) base.dataStudioMetrics = skill.dataStudioMetrics;
  if (skill.dataStudioExampleQuestions?.length) {
    base.dataStudioExampleQuestions = skill.dataStudioExampleQuestions;
  }
  if (skill.dataStudioPermissionHint) {
    base.dataStudioPermissionHint = skill.dataStudioPermissionHint;
  }
  if (skill.dataStudioMcpUrl) base.dataStudioMcpUrl = skill.dataStudioMcpUrl;
  if (skill.dataStudioTimeField) base.dataStudioTimeField = skill.dataStudioTimeField;
  if (skill.dataStudioDimensions?.length) {
    base.dataStudioDimensions = skill.dataStudioDimensions;
  }
  if (skill.dataStudioEvidence?.length) base.dataStudioEvidence = skill.dataStudioEvidence;
  return base;
}
...
function toConfig(draft: AgentDraft): Record<string, unknown> {
  ...
  if (draft.dataAssets?.length)
    o.dataAssets = draft.dataAssets.map(toDataAssetConfig);
