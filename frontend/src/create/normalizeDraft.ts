...
function parseDataAssets(o: Record<string, unknown>): SelectedSkill[] {
  if (!Array.isArray(o.dataAssets)) return [];
  const out: SelectedSkill[] = [];
  for (const raw of o.dataAssets as unknown[]) {
    const so = (raw && typeof raw === "object" ? raw : {}) as Record<string, unknown>;
    const assetType = asString(so.dataStudioAssetType);
    const assetId = asString(so.dataStudioAssetId).trim();
    if ((assetType !== "dashboard" && assetType !== "semantic_model") || !assetId) {
      continue;
    }
    const name = asString(so.name) || assetId;
    const folder = asString(so.folder) || `datastudio-${assetType}-${assetId}`;
    const score = so.dataStudioGateScore;
    out.push({
      source: "datastudio",
      folder,
      name,
      ...
      dataStudioGateScore: typeof score === "number" ? score : undefined,
      ...
      dataStudioMcpUrl: asString(so.dataStudioMcpUrl),
