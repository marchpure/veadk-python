...
test("OpenViking is selected through StudioApp instead of bypassing the shell", () => {
  assert.match(appSource, /rootWorkspaceFromLocation/);
  assert.match(appSource, /window\.addEventListener\("popstate", syncRootWorkspace\)/);
  assert.match(appSource, /return <StudioApp initialWorkspace=\{rootWorkspace\} \/>/);
  assert.doesNotMatch(
    appSource,
