...
test("semantic graph helper renders fixture MDL as nodes edges minimap controls canvas", async () => {
  const { buildSemanticGraph } = await loadTsxModule(
  ...
  const graph = buildSemanticGraph({
    ...
    relationships: [{ id: "sales_store", from: "sales", to: "store" }],
  });
...
test("workbench CSS avoids horizontal page overflow and nested card shells on mobile", () => {
  ...
  assert.match(cssSource, /\.kc-askdash-split\s*\{[\s\S]*?grid-template-columns: minmax\(320px, 0\.38fr\) 8px minmax\(520px, 0\.62fr\)/);
