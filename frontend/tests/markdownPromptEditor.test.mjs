...
test("skill sources open in a fixed-height dialog above a six-row selected list", () => {
  ...
  assert.match(createSource, /id: "datastudio", label: "知识资产", icon: Database/);
  assert.match(createSource, /dataAssets=\{dataAssets\}/);
  assert.match(createSource, /onDataAssetsChange=\{\(next\) =>\s*patch\(\{ dataAssets: next \}\)/);
  assert.match(createSource, /active === "datastudio"[\s\S]*?<DataStudioAssetPicker/);
  assert.match(
    createSource,
    /\{ id: "local", label: "本地文件"[\s\S]*?\{ id: "skillspace", label: "AgentKit Skills 中心"[\s\S]*?\{ id: "skillhub", label: "火山 Find Skill 技能广场"[\s\S]*?\{ id: "datastudio", label: "知识资产"/,
  );
  ...
  assert.match(
    createSource,
    /\{allSelected\.length > 0 && \([\s\S]*?className="cw-selected-skill-list"[\s\S]*?role="dialog"/,
  );
