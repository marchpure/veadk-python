...
@pytest.mark.asyncio
async def test_datastudio_asset_materialization_writes_skill_md() -> None:
    draft = AgentDraft(
        name="demo",
        instruction="You are helpful.",
        dataAssets=[
    ...
    assert [file.path for file in project.files] == [
        "skills/datastudio-dashboard-sales/SKILL.md"
    ]
    skill_md = project.files[0].content
    assert "name: datastudio-dashboard-sales" in skill_md
    assert "GMV" in skill_md
    assert "pay_date" in skill_md
    assert "Aggregated metrics only" in skill_md
    assert "回答必须附上工具返回的 SQL" in skill_md
