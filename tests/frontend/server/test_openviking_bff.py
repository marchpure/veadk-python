...
def test_resource_refs_are_signed_profile_scoped_and_workspace_scoped() -> None:
    ...
    with pytest.raises(OpenVikingError, match="outside workspace"):
        service.resource_ref(first, "viking://resources/workspace-a-private/secret.md")
...
def test_base_url_dev_http_only_allows_loopback_hosts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def resolved_address(host: str, port: int, *, type: int):
        if host in {"localhost", "127.0.0.1", "::1"}:
            return [(socket.AF_INET, type, 6, "", ("127.0.0.1", port))]
        if host == "dev.internal":
            return [(socket.AF_INET, type, 6, "", ("10.0.0.2", port))]
        return [(socket.AF_INET, type, 6, "", ("8.8.8.8", port))]

    monkeypatch.setattr(socket, "getaddrinfo", resolved_address)
    service = OpenVikingService(OpenVikingProfileRepository(), config())

    cloud_profile(service, "http://127.0.0.1:38110")
    cloud_profile(service, "http://[::1]:38110")
    with pytest.raises(OpenVikingError) as private_http:
        cloud_profile(service, "http://dev.internal:38110")
    with pytest.raises(OpenVikingError) as public_http:
        cloud_profile(service, "http://api.vikingdb.cn-beijing.volces.com")

    assert private_http.value.code == "INVALID_BASE_URL"
    assert public_http.value.code == "INVALID_BASE_URL"


def test_profile_credentials_revalidate_base_url_on_use(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    addresses = ["8.8.8.8"]

    def resolved_address(_host: str, port: int, *, type: int):
        return [(socket.AF_INET, type, 6, "", (addresses[0], port))]

    monkeypatch.setattr(socket, "getaddrinfo", resolved_address)
    service = OpenVikingService(OpenVikingProfileRepository(), config())
    value = cloud_profile(service, "https://api.vikingdb.cn-beijing.volces.com")

    addresses[0] = "169.254.169.254"
    with pytest.raises(OpenVikingError) as raised:
        service._credentials(value)
...
@pytest.mark.asyncio
async def test_profile_routes_require_validated_profile_before_operations() -> None:
    app = FastAPI()
    calls = 0
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as bff:
        response = await bff.post(path, json={"payload": {}})

    assert response.status_code == 409
    assert response.json()["detail"]["code"] == "OPENVIKING_PROFILE_NOT_READY"
    assert calls == 0
    await client.aclose()
...
async def test_connection_resource_import_reauthorizes_and_strips_secrets() -> None:
    ...
    openviking.repository.save(
        openviking_profile.__class__(
            **{**openviking_profile.__dict__, "status": "ready"}
        )
    )
