...
def test_upload_persists_source_golden_chunks_and_skill_draft(tmp_path):
    ...
    created = service.create_knowledge_base("workspace", "Policies", "Sales policies", "personal")
    ...
    assert result["goldenAssetRevision"]["sourceRevisionId"] == result["sourceRevision"]["id"]
    ...
    assert summary["sources"][0]["goldenAssetRevision"]["sourceRevisionId"] == result["sourceRevision"]["id"]
...
def test_semantic_invalid_revision_is_rejected_and_valid_revision_has_diff(tmp_path):
    ...
    first = service.save_semantic("semantic_sales", "model Sales {\n dimension region : string\n}", 0)
    ...
    assert service.get_semantic("semantic_sales")["goldenSchema"]["models"][0]["name"] == "Sales"


def test_semantic_source_revision_persists_inferred_golden_schema_and_rejects_unknown_fields(tmp_path):
    service = DomainService(tmp_path / "domains.sqlite3")
...
def test_graph_mutation_and_query_are_durable(tmp_path):
    service = DomainService(tmp_path / "domains.sqlite3")
    service.mutate_graph("graph_sales", {
        "operation": "upsert_entity", "entity": {"id": "customer", "type": "Customer"},
    })
    service.mutate_graph("graph_sales", {
        "operation": "upsert_entity", "entity": {"id": "order", "type": "Order"},
    })
    mutated = service.mutate_graph("graph_sales", {
        "operation": "upsert_relationship",
        "relationship": {"id": "places", "from": "customer", "to": "order", "type": "PLACES"},
    })
    assert mutated["revision"] == 3
    queried = service.query_graph("graph_sales", {"mode": "path", "from": "customer", "to": "order"})
    assert queried["relationships"][0]["id"] == "places"
    assert queried["entities"] == ["customer", "order"]
    assert queried["queryResultId"]
    assert service.graph_query_result("graph_sales", queried["queryResultId"]) == queried
    assert service.graph("graph_sales")["revision"] == 3
    with pytest.raises(ValueError):
        service.mutate_graph("graph_sales", {
            "operation": "upsert_relationship",
            "relationship": {"id": "invalid", "from": "customer", "to": "missing", "type": "PLACES"},
        })
...
def test_http_graph_path_query_preserves_from_alias(tmp_path):
    ...
    for entity_id, entity_type in (("customer", "Customer"), ("order", "Order")):
        response = client.post(
            ...
            json={"operation": "upsert_entity", "entity": {"id": entity_id, "type": entity_type}},
        )
...
def test_http_query_result_and_semantic_source_revision_round_trips(tmp_path):
    ...
    source = client.post(
        ...
        data={"title": "Sales", "description": "", "tags": "", "chunk_strategy": "fixed"},
    )
    assert source.status_code == 200
    source_revision_id = source.json()["sourceRevision"]["id"]
    source_revisions = client.get(
        "/api/knowledge-domains/v1/semantic-source-revisions"
    )
...
def test_http_upload_and_credential_blocked_feishu(tmp_path):
    ...
    uploaded = client.post(
        f"/api/knowledge-domains/v1/knowledge-bases/{created.json()['id']}/sources",
        files={"file": ("policy.md", b"Returns require an order receipt.", "text/markdown")},
        data={"title": "Policy", "description": "", "tags": "", "chunk_strategy": "auto"},
    )
