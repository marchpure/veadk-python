...
def _build_agent(acc: _Acc, draft: AgentDraft, var_name: str) -> str:
    ...
    for mcp_tool in draft.mcpTools:
        if mcp_tool.transport == "http" and mcp_tool.url.strip():
            _add_import(
                acc, "from veadk.tools.mcp_tool.trusted_mcp_toolset import TrustedMcpToolset"
            )
            _add_import(
                acc,
            ...
            if mcp_tool.authTokenEnv.strip():
                ...
                auth_var = f"{v}_auth_token"
                headers_var = f"{v}_headers"
                acc.pre_lines.append(f"{auth_var} = os.getenv({_py_str(env_name)}, \"\")")
                acc.pre_lines.append(
                    f'{headers_var} = {{"Authorization": f"Bearer {{{auth_var}}}"}} '
                    f"if {auth_var} else None"
                )
                headers = f", headers={headers_var}"
                acc.env.append(
                    EnvVar(
                        env_name,
                        True,
            ...
            acc.pre_lines.append(
                f"{v} = TrustedMcpToolset(connection_params=StreamableHTTPConnectionParams("
            ...
            acc.pre_lines.append(
                f"{v} = TrustedMcpToolset(connection_params=StdioConnectionParams("
    ...
    skill_folders = list(
        dict.fromkeys(
            skill.folder
            for skill in [*draft.selectedSkills, *draft.dataAssets]
            if skill.folder.strip()
        )
    )
    if skill_folders:
