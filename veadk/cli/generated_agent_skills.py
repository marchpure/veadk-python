...
async def materialize_selected_skills(
    ...
    *,
    ...
    for skill in _collect_selected_skills(draft):
        ...
        elif skill.source == "datastudio":
            files = _materialize_datastudio_asset(skill)
        else:
...
def _collect_selected_skills(draft: AgentDraft) -> list[SelectedSkill]:
    ...
    def visit(node: AgentDraft) -> None:
        for skill in [*node.selectedSkills, *node.dataAssets]:
...
def _materialize_datastudio_asset(skill: SelectedSkill) -> list[GeneratedFile]:
    folder = _safe_folder(skill.folder or skill.name or skill.dataStudioAssetId)
    asset_type = skill.dataStudioAssetType
    asset_id = skill.dataStudioAssetId.strip()
    if asset_type not in {"dashboard", "semantic_model"} or not asset_id:
        raise DebugPolicyError("Data Studio asset is missing identity")
    skill_md = _render_datastudio_skill_md(skill, folder)
    _normalize_skill_md_frontmatter(skill_md, f"Data Studio asset {asset_id}")
    return [GeneratedFile(path=f"skills/{folder}/SKILL.md", content=skill_md)]
...
def _render_datastudio_skill_md(skill: SelectedSkill, folder: str) -> str:
    name = skill.name.strip() or skill.dataStudioAssetId
    description = (
        skill.description.strip()
        or f"Use Byaan Data Studio asset {name} for governed analytics answers."
    )
    metrics = _bullet_list(skill.dataStudioMetrics, "资产未声明指标清单，查询前先读取资产元数据。")
    dimensions = _bullet_list(skill.dataStudioDimensions, "资产未声明维度清单，查询前先读取资产元数据。")
    examples = _bullet_list(skill.dataStudioExampleQuestions, "围绕该资产覆盖的指标、维度和时间范围回答问题。")
    evidence_examples = _bullet_list(
        skill.dataStudioEvidence,
        "如果资产没有返回证据明细，先调用工具读取可引用的口径或面板信息。",
    )
    permission = (
        skill.dataStudioPermissionHint.strip()
        or "只允许使用当前用户有权限访问的聚合数据；不得导出明细或绕过脱敏策略。"
    )
    time_field = skill.dataStudioTimeField.strip() or "使用资产元数据声明的默认时间字段。"
    version = skill.dataStudioVersion.strip() or "published"
    gate = f"{skill.dataStudioGateScore:g}" if skill.dataStudioGateScore is not None else "未提供"
    return f"""---
name: {folder}
description: {_yaml_string(description[:1000])}
---

# {name}

This skill represents the Byaan Data Studio asset `{skill.dataStudioAssetType}/{skill.dataStudioAssetId}`.
...
- {permission}
- Do not expose raw rows, masked fields, or data outside the user's policy scope.

## Evidence Requirement

- 回答必须附上工具返回的 SQL、图表口径、文档章节或权限策略证据。
{evidence_examples}

When answering with this asset, call the paired Byaan MCP tool, ground the answer in the returned evidence, and cite the SQL, metric definition, dashboard panel, or permission policy that supports the conclusion.
"""
